...
class UserApiIntegrationTest extends IntegrationTestBase {
    ...
    void shouldReturnAllUsers_whenGettingAllUsers() throws Exception {
        UserEntity user1 = existingUser(generateUser());
        UserEntity user2 = existingUser(generateUser());
    ...
    void shouldReturnDetailsAboutUser_whenGettingUserByEmail() throws Exception {
        ...
        mockMvc.perform(get("/v1/users/by-email").param("email", user1.getEmail()).contentType(MediaType.APPLICATION_JSON))
                .andDo(log())
                .andExpect(status().isOk())
    ...
    void shouldReturnAllUsersOlderThan_whenGettingAllUsersOlderThan() throws Exception {
        UserEntity user1 = existingUser(generateUserWithDate(LocalDate.of(2000, 8, 11)));
        UserEntity user2 = existingUser(generateUserWithDate(LocalDate.of(2024, 8, 11)));
    ...
    void shouldRemoveUserFromRepository_whenDeletingClient() throws Exception {
        ...
        mockMvc.perform(delete("/v1/users/delete/{id}", user1.getId())
                        .contentType(MediaType.APPLICATION_JSON))
                .andDo(log())
                .andExpect(status().isNoContent());

        List<UserEntity> allUser = getAllUsers();
    ...
    public static UserEntity generateUser() {
        return new UserEntity(randomUUID().toString(), randomUUID().toString(), LocalDate.now(), randomUUID().toString());
    }

    private static UserEntity generateUserWithDate(LocalDate date) {
        return new UserEntity(randomUUID().toString(), randomUUID().toString(), date, randomUUID().toString());
    }
